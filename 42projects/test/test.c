...
bool isValidMap(const char *buffer, MapInfo *mapInfo, size_t bufferSize) {
    ...
    // Check if the last line was properly terminated
    if (currentLineLength > 0) {
        lineCount++;
        if (expectedLineLength == 0) {
            expectedLineLength = currentLineLength;
    ...
    // Validate at least one non-empty line and proper line breaks
    return hasNonEmptyLine && validCharacters && lineCount == mapInfo->numLines;
}
